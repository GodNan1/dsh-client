...
- **插件管理**：主窗口点「插件」即可安装 DSH 插件——支持 `github:用户/仓库`、npm 包名、本地目录；自动运行 `dsh plugin --profile web add`，显示实时输出；Git 插件被 pnpm 阻止构建脚本时会**自动写入 `onlyBuiltDependencies` 许可并重试**；列表选中即可卸载（双击也可），装完/卸完可一键重启服务
- **安装 DSH（新环境）**：设置里点「安装 DSH…」——标准安装流程：选目录 → 检查环境（git/node/pnpm）→ 克隆官方仓库 → `pnpm install` → `pnpm run build` → 自动把客户端指向新目录；新电脑上装好 node + git 后一键完成
- **托盘常驻**：最小化窗口即隐藏到托盘；图标颜色实时反映状态（绿=运行中、灰=已停止）；双击托盘弹出主窗口，右键菜单可启动/停止/看日志/退出
