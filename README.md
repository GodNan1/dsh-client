...
- **插件管理**：主窗口点「插件」即可安装 DSH 插件——支持 `github:用户/仓库`、npm 包名、本地目录；自动运行 `dsh plugin --profile web add`，显示实时输出；Git 插件被 pnpm 阻止构建脚本时会**自动写入 `onlyBuiltDependencies` 许可并重试**；窗口里能看到已安装的插件层
